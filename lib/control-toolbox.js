...
cst.colon = function (t_start,dt,t_end){
    ...
    var t_cond = (t_start < t_end) ? function(t,c){return (t <= c);} : function(t,c){return (t > c);};
    var t_now = t_start;
    var t = [];
...
cst.ss.prototype.lsim = function (u, t, x0) {
    ...
    if (!Array.isArray(u)) {
        u = math.add(u, cst.zeros(t.length, 1));
    } else {
        u = cst.to_colv(u);
    }
    ...
    // Get submatrices
    var expMsize = expM.size();
    var Ad = expM.subset(math.index(math.range(0, nn), math.range(0, nn)));
    var Bd1 = expM.subset(math.index(math.range(0, nn), math.range(nn+ni, expMsize[1])));
    var Bd0 = expM.subset(math.index(math.range(0, nn), math.range(nn, nn+ni)));
    var Bd01 = math.add(Bd0, math.multiply(-1, Bd1));
    
    // Iterations
    var xout = cst.zeros(nn, ns-1);
    xout = math.concat(x0, xout);
