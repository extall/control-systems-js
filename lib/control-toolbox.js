...
// Convert array or row vector to column vector
cst.to_colv = function (a) {
    // Is it array or vector (latter has a shape like (1,N) or (N,1)?
    if (a[0][0] === undefined)
    {
        // Array
        ...
        // Vector, but need to make sure just in case
        if (a.matsize()[0] !== 1 && a.matsize()[1] !== 1){
            throw("Can only operate on vectors, but a matrix is supplied.");
        }

        // If it is a vector, just return the vector if it is already a column vector
        if (a.matsize()[1] == 1){
            return a;
        }else{
            // Otherwise convert a row vector to a column vector
            return math.reshape(a, [a.matsize()[1], 1]);
        }
