...
csg.draw_timeseries = function (t, sel, params = undefined, background = undefined) {
    ...
    // Is there already a precomputed u?
    var u = undefined;
    if (params !== undefined && params.u !== undefined){
        u = params.u;
    }
    else
    {
        u = cst.zeros(t.matsize()[0], 1);
    }
    ...
    // Set: sets background points with index. No check on points is performed
    drawable_plot.userdata.set_background_points = function(index, points){
        if (this.handle.options.data[index]["points"] !== undefined){
            this.handle.options.data[index]["points"] = points;
        }
    };
    ...
    drawable_plot.on("mousemove", function (params) {
        ...
        // TODO: There is a bug that appears when the underlying object is scaled
        // TODO: See the following issue: https://github.com/mauriciopoppe/function-plot/issues/53
        var x = params["x"];
        var y = params["y"];
...
// TODO: right now, tf is allowed to be an object with entries obj.mag, obj.ph, obj.w.
// TODO: This is not very nice in general, but javascript of course has its limitations ...
csg.bode = function (tf, w, sel, params) { // TODO: should be system type agnostic (support for any lti)
    ...
    if (tf.mag !== undefined && tf.ph !== undefined && tf.w !== undefined){
        // This is the case of an object with magnitude, phase, and freq passed
        // instead of G. That means we can easily plot this data instead of computing it for TF
        w = [tf.w];
        magr = tf.mag;
        phr = tf.ph;
        data1 = math.concat(math.transpose(w), math.transpose([tf.mag]));
        data2 = math.concat(math.transpose(w), math.transpose([tf.ph]));
    }
    else{
        ...
        magr = mp[0];
        phr = mp[1];
        data1 = math.concat(math.transpose(w), math.transpose([mp[0]]));
        data2 = math.concat(math.transpose(w), math.transpose([mp[1]]));
    }
    ...
    var fig1 = csg.plot({
        ...
        yAxis: {
            label: 'Magnitude [dB]',
            domain: csg.autorange_y(magr, [-10, 10])
        },
        grid: true,
        disableZoom: true,
        data: [{
    ...
    var fig2 = csg.plot({
        ...
        yAxis: {
            label: 'Phase [deg]',
            domain: csg.autorange_y(phr, [-10, 10])
        },
        grid: true,
        disableZoom: true,
        data: [{
