...
csg.timeseries = function(t, u, sel, params=undefined){
    ...
    var toplot = math.concat(math.transpose(t), math.transpose(u));
    ...
    var fig1 = csg.plot({
        ...
        xAxis: {
            label: xlabel,
            domain: csg.autorange_x(t)
        },
        yAxis: {
            label: ylabel,
            domain: csg.autorange_y(u)
        },
        grid: true,
        disableZoom: true,
        data: [{
...
csg.DualMatrixTable = function(){
    ...
    this.rows = new Array(); // All rows are stored in a multidimensional array
